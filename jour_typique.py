...
P_sol_unit = eta_p * A_p * (G / 1000.0) * delta_t  # conversion W/m² -> kW/m²
...
if model.Status == GRB.OPTIMAL:
    ...
    # Valeurs d'état de charge
    S_vals = np.array([S[t].X for t in range(T + 1)])
    SoC_pct = S_vals / (C_b * Nb_opt) * 100

    # Graphique de production / demande
    hours = np.arange(T)
    ...
    plt.xlabel("Heure")
    plt.ylabel("kWh")
    plt.title("Journée type - production et demande")
    plt.legend()
    plt.grid(alpha=0.3)
    plt.tight_layout()
    plt.savefig("jour_typique_resultats.png", dpi=150)
    print("Graphique sauvegardé : jour_typique_resultats.png")

    # Graphique de l'état de charge des batteries
    ...
    plt.grid(alpha=0.3)
    plt.tight_layout()
    plt.savefig("jour_typique_soc.png", dpi=150)
    plt.show()
    print("Graphique sauvegardé : jour_typique_soc.png")
else:
